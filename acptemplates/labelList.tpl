				...
				{content}
					{foreach from=$objects item=label}
						<tr class="labelRow">
							...
							<td class="columnTitle columnLabel">{if $label->isEditable()}<p class="label{if $label->cssClassName} {$label->cssClassName}{/if}"><a href="{link controller='LabelEdit' id=$label->labelID}{/link}">{lang}{$label->label}{/lang}</a>{else}{lang}{$label->label}{/lang}</p>{/if}</td>
